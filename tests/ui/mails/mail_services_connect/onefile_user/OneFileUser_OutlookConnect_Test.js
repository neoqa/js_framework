...
function testBody(connect_Page) {
  ...
  BaseTest.test.it("Should connect to microsoft-outlook mail server", () => {
    connect_Page = CommonActions.passingPage;
    let mails_Page = connect_Page.mailClient_Connect('OUTLOOK');
    mails_Page.pageWait();
...
// Initiate test run
BaseTest.testIt("Log in with 1File account and connect to Outlook", testBody);